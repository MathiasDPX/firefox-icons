...
class AppIcon:
    def __init__(self,
        ...
        fg_content = _extract_svg_content(self.foreground.svg)
        bg_content = _extract_svg_content(self.background.svg)
        fg_viewbox = _extract_viewbox(self.foreground.svg)
        ...
        self.svg = f'<svg xmlns="http://www.w3.org/2000/svg" width="{size}" height="{size}" viewBox="0 0 {size} {size}">{bg_content}<g transform="translate({offset_x},{offset_y}) scale({scale_x},{scale_y})">{fg_content}</g></svg>'
